...
import { z } from "zod";
import { loginWithCredentials, loginWithPlaywright, saveSession, loadSession, clearSession, } from "./auth.js";
import { searchProblems, getProblem, getDailyChallenge, getSubmissions, submitSolution, waitForResult, checkSubmission, getUserProfile, SessionExpiredError, } from "./leetcode-client.js";
import { LANGUAGE_SLUGS } from "./types.js";
import { getHint, reviewApproach, explainStepByStep, debugSolution, analyzeComplexity, } from "./ai-helper.js";
...
server.tool("reauth", "Open a real browser window so you can log in to LeetCode normally — no copy-pasting cookies. The window closes automatically once login is detected.", {}, async () => {
    try {
        const session = await loginWithPlaywright();
        return {
            content: [
...
server.tool("logout", "Clear the saved LeetCode session", {}, async () => {
    await clearSession();
    return { content: [{ type: "text", text: "Session cleared." }] };
});
server.tool("whoami", "Show your LeetCode profile and solve counts", {}, async () => {
...
server.tool("status", "Check if the MCP server is running and whether your LeetCode session is active", {}, async () => {
    const session = await loadSession();
    if (!session) {
        return {
            content: [
            ...
            content: [
                {
                    ...
                    text: [
                        "MCP server: ✓ running",
                        `LeetCode:   ✗ ${expired ? "session expired" : "could not verify — " + err.message}`,
                        ...(expired
                            ? [
                                "",
...
// ── Problem Tools ─────────────────────────────────────────────────────────────
server.tool("search_problems", "Search for LeetCode problems by keyword, with optional difficulty filter", {
    ...
    try {
        ...
        const lines = problems.map((p) => `#${p.frontendId.padStart(4, " ")} [${p.difficulty}]  ${p.title}` +
            (p.status === "ac" ? " ✓" : "") +
            `  (${p.acRate}% AC)` +
            `  slug: ${p.titleSlug}`);
...
server.tool("get_problem", "Fetch the full problem statement, examples, hints, and starter code for all languages", {
    title_slug: z
        .string()
        .describe('The slug from the LeetCode URL, e.g. "two-sum" or "merge-intervals". ' +
        'Use search_problems to find the slug if unsure.'),
    language: z
        .string()
        .optional()
        .describe('Return starter code only for this language (e.g. "python", "typescript", "java"). ' +
...
server.tool("daily_challenge", "Get today's LeetCode daily challenge problem", {
    language: z
        .string()
        .optional()
...
// ── Submission Tools ──────────────────────────────────────────────────────────
server.tool("my_submissions", "List your recent submission history, optionally filtered to a specific problem", {
    title_slug: z
        .string()
        .optional()
        .describe('Filter to one problem, e.g. "two-sum". Omit for your recent submissions across all problems.'),
    limit: z
        .number()
        .int()
        .min(1)
...
server.tool("submit_solution", [
    ...
    try {
        ...
        const submissionId = await submitSolution(session, title_slug, problem.questionId, langSlug, code);
        // Wait for the result
        ...
        if (result.memory)
            lines.push(`Memory: ${result.memory}`);
        ...
        if (result.statusMsg !== "Accepted" && result.lastTestcase) {
            ...
            if (result.codeOutput)
                lines.push(`Got:      ${result.codeOutput}`);
        }
        return { content: [{ type: "text", text: lines.join("\n") }] };
    }
    catch (err) {
...
server.tool("check_submission", "Check the result of a submission by its ID (useful if submit_solution timed out)", {
...
server.tool("review_approach", "Describe your intended approach and get feedback on correctness, complexity, and whether a better solution exists — without being handed the answer.", {
    title_slug: z.string().describe('Problem slug, e.g. "two-sum"'),
    approach: z
        .string()
        .describe("Describe your approach in plain English, e.g. 'I want to use a hash map to store seen values and check for complements'"),
...
server.tool("explain_step_by_step", "Get a full conceptual walkthrough of how to think about and solve a problem — covers the pattern, algorithm steps, and edge cases. No code given.", {
    title_slug: z.string().describe('Problem slug, e.g. "two-sum"'),
    focus: z
        .string()
        .optional()
        .describe('Optional: focus on a specific part, e.g. "how to handle duplicates" or "the sliding window part"'),
...
server.tool("debug_solution", "Paste your failing code and the error or wrong test case — get help finding the bug without being handed a rewrite.", {
    title_slug: z.string().describe('Problem slug, e.g. "two-sum"'),
    language: z.string().describe('Language of the code, e.g. "python3", "typescript"'),
    code: z.string().describe("Your current solution code"),
    error: z
        .string()
        .describe('The error message, wrong answer, or failing test case. e.g. "Expected 3, got 2 for input [1,2,3]"'),
...
server.tool("analyze_complexity", "Analyze the time and space complexity of your solution and learn if a better complexity is achievable for this problem.", {
    title_slug: z.string().describe('Problem slug, e.g. "two-sum"'),
    language: z.string().describe('Language of the code, e.g. "python3", "typescript"'),
