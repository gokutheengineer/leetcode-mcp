...
export async function loginWithCredentials(username, password) {
    ...
    if (!csrfToken) {
        throw new Error("Could not obtain CSRF token from LeetCode login page. " +
    ...
    if (!leetcodeSession) {
        ...
        if (body.includes("recaptcha") || body.includes("captcha")) {
            throw new Error("LeetCode blocked the login with reCAPTCHA. " +
...
export async function loginWithPlaywright() {
    ...
    try {
        username = await page.evaluate(() => {
            ...
            const avatar = document.querySelector('img[alt*="avatar"], img[class*="avatar"]');
