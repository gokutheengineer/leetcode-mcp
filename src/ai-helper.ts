...
/**
 * A thin wrapper around MCP sampling — the host (Claude Code, Cursor, etc.)
 * handles the actual API call using its own credentials. No API key needed.
 */
export type Sampler = (
  systemPrompt: string,
  userMessage: string,
...
function problemContext(p: ProblemDetail): string {
  return [
